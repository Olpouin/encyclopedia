...
foreach ($config['cardsList'] as $type => $groups) {
	$sidenavContent .= '<ul class="type"><span><a href="'.$config['general']['path'].'/'.$type.'/">'.ucfirst($config['types'][$type]).'</a></span>';
	foreach ($groups as $group => $cards) {
		...
		foreach ($cards as $card => $data) {
			if ($data['hidden'] == 0) {
				...
				$sidenavContent .= '<li><a class="'.$selectedClass.'" href="'.$config['general']['path'].'/'.$type.'/'.URLConvert($card).$editURL.'" title="'.$card.'">'.$card.'</a></li>';
			}
		}
		$sidenavContent .= '</ul>';
	}
